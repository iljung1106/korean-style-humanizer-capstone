#!/usr/bin/env python3
"""Stage 8E short mixed GRPO from Stage 8D."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train pipeline_v2 Stage 8E short mixed GRPO.")
    add_common_grpo_args(parser, task="mixed", default_output_name="stage08e_grpo_mixed_from_stage08d")
    ...
    parser.set_defaults(
        ...
        output=str(TRAINING_ROOT / "outputs" / "pipeline_v2" / "stage08e_grpo_mixed_from_stage08d"),
        adapter_path=str(TRAINING_ROOT / "outputs" / "pipeline_v2" / "stage08d_grpo_rewrite_from_stage08c" / "policy"),
        ...
        rewrite_style_base_weight=0.75,
        rewrite_anti_slop_family_weight=0.22,
        rewrite_translationese_family_weight=0.0,
        rewrite_comma_family_weight=0.03,
        rewrite_pos_family_weight=0.20,
        rewrite_modifier_family_weight=0.14,
        rewrite_lexical_family_weight=0.0,
        rewrite_sentence_edge_family_weight=0.28,
        rewrite_sentence_length_family_weight=0.40,
        rewrite_edit_weight=0.20,
        rewrite_improvement_weight=0.42,
        rewrite_improvement_scale=0.25,
        rewrite_low_edit_penalty_max=0.10,
        ...
        long_output_penalty=0.05,
        collapse_fail_reward=-0.60,
        max_steps=24,
        ...
        group_diversity_bonus_max=0.02,
...
def main() -> None:
    run_grpo_stage(parse_args(), stage="stage08e_grpo_mixed_from_stage08d", task="mixed")
